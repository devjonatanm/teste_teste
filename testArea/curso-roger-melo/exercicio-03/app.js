...
const femaleDogNames = ['Lilica', 'Matilda', 'Carol']
...
for (let i = 0; i < dogNames.length; i++) {
    dogNames[i].charAt(0)
    initials.push(dogNames[i].charAt(0))
}
...
let juncao = dogNames[0].slice(dogNames[0].length - 2, dogNames[0].length)
    + dogNames[1].slice(dogNames[1].length - 2, dogNames[1].length)
    + dogNames[5].slice(dogNames[5].length - 2, dogNames[5].length)
let rep = juncao.replace('n', 'd')

//console.log('A '+rep+' é um doce à base de coco, tradicional na América Latina e em Angola.')
